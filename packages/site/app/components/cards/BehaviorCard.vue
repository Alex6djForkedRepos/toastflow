    ...
    >
      <CardLayout class="gap-3">
        <InputField
          ...
          @update:model-value="
            emit('update:buttonsAlignment', $event as ToastButtonsAlignment)
          "
        />
        ...
        <InputField
          ...
          @update:model-value="
            emit('update:buttonsLayout', $event as ToastButtonsLayout)
          "
        />
        ...
        <div class="grid grid-cols-2 gap-2">
          <InputField
            ...
            placeholder="e.g. 6pxâ€¦"
            @update:model-value="emit('update:buttonsGap', String($event))"
          />
          <InputField
            label="Content gap"
            :model-value="buttonsContentGap"
            placeholder="e.g. 10pxâ€¦"
            @update:model-value="
              emit('update:buttonsContentGap', String($event))
            "
          />
        </div>
            ...
            >
              ...
              >
                ...
                <div class="flex flex-wrap items-center gap-2">
                  <Button
                    ...
                    tooltip="Dismiss before onClick"
                    @update:model-value="
                      emitButtonUpdate(button.id, { dismissOnClick: $event })
                    "
                    class="px-2! py-0.5!"
                  >
                    Dismiss before
                  ...
                  <Button
                    variant="toggle"
                    :model-value="button.dismissAfterClick"
                    tooltip="Dismiss after onClick"
                    @update:model-value="
                      emitButtonUpdate(button.id, { dismissAfterClick: $event })
                    "
                    class="px-2! py-0.5!"
                  >
                    Dismiss after
              ...
              <div class="grid gap-2 sm:grid-cols-3">
                <InputField
                  ...
                  class="sm:col-span-1"
                  @update:model-value="
                    emitButtonUpdate(button.id, {
                      mode: $event as PlaygroundButtonMode,
                    })
                  "
                />
                ...
                <InputField
                  ...
                  :label="
                    button.mode === 'html' ? 'HTML content' : 'Button label'
                  "
                  :model-value="button.value"
                  :placeholder="
                    button.mode === 'html'
                      ? '<strong>Details</strong>â€¦'
                      : 'Button labelâ€¦'
                  "
                  @update:model-value="
                    emitButtonUpdate(button.id, { value: String($event) })
                  "
                />
              </div>
              ...
              <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <InputField
                  ...
                  placeholder="e.g. btn-primaryâ€¦"
                  @update:model-value="
                    emitButtonUpdate(button.id, { className: String($event) })
                  "
                />
                <InputField
                  label="ariaLabel (optional)"
                  :model-value="button.ariaLabel"
                  placeholder="e.g. Open detailsâ€¦"
                  @update:model-value="
                    emitButtonUpdate(button.id, { ariaLabel: String($event) })
                  "
                />
              </div>
            </div>
